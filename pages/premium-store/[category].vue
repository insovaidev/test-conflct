                        ...
                        <div v-else>
                            ...
                            <div v-else :class="mobileDevice ? 'py-5' : 'main_min_height px-3 mt_80'" >
                                <div :class="mobileDevice ? 'pt-3 px-3 pb-5' : ''">
                                        ...
                                        <div @click="activePlan = index, focusAndMoveToMiddle(plan[0].plan)" :id="'id'+plan[0].plan" :key="plan" v-for="(plan, index) in planByCategoryGrouped" class="other_plan_by_category_card" :class="{ 'currentActivePlan': planByCategory.subscription_plan && planByCategory.subscription_plan.plan == index, 'active': activePlan == index}">
                                            <div @click="choosePlan(plan, index)" :class="isDesktop ? 'other_plan_by_category_card_content' : 'other_plan_by_category_card_content_mobile'">
                                                ...
                                                <div class="other_plan_by_category_card_term term_active">
                                                    ...
                                                    <div v-else class="other_plan_term_card_view " :class="[(activePlan != index && planByCategory.subscription_plan && planByCategory.subscription_plan.plan != index) ? 'active': '']">
                                    ...
                                    <div v-if="!isLoading && isDesktop" v-show="subScribeFormData.planTitle">
                                        ...
                                        <button type="button" class="btn form-control-lg btn-xm text-white btn-k24-secondary subscribe_button_footer" @click="storeData(route.params.category), router.push(localePath({ name: 'checkout', query: {'storeid': route.query.id }}))">
                                            {{ $t(`account_t.subscribe_now`)}}   
                                        </button>
                                        </div>
                                    </div>
                                    <div v-if="!isLoading && !isDesktop" v-show="subScribeFormData.planTitle" class="subscribe_button_footer_wrapper">
                                        <div class="subscribe_button_footer_term_wrapper">
                                            <div class="subscribe_button_footer_term_left">
                                                <p class="mb-0 subscribe_button_footer_term_plan">{{ subScribeFormData.planTitle ? $t(`account_t.${subScribeFormData.planTitle}`)+ ':' : '' }} </p>
                                                <p class="mb-0 subscribe_button_footer_term">{{ subScribeFormData.term ? subScribeFormData.term.term : '' }}</p>
                                        ...
                                        <button type="button" class="btn form-control-lg btn-xm btn-k24-secondary subscribe_button_footer" @click="storeData(route.params.category), router.push(localePath({ name: 'checkout',  query: {'storeid': route.query.id }}))" >
                                            {{ $t(`account_t.subscribe_now`)}}       
                                        </button>
                                    </div> 
                                </div>
...
const user_data = ref('');
const planByCategory = useState('planByCategory', () => '') // plan by category Grouped By plan
...
useHead({
    title: 'Premium Store - Khmer24.com', // set header title
    meta: [
...
async function getPlanByCategory() {
    ...
    try {
        const response = await $fetch(`${config.VUE_APP_API_URL_PAYMENT}services/store/${route.params.category}?lang=${locale.value}`, { 
            retry: 2, 
            retryDelay: 3000, 
            query: query,
            method: "GET", 
...
function storeData(plan_category=null) {
    ...
    localStorage.setItem('cart_data', JSON.stringify({'id': id, 'type': 'store', 'plan': subScribeFormData.termId, 'category': plan_category}))
}
