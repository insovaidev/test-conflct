...
<template>
    <div>
        ...
        <div class="p-search mt_80 min_h max_width_form">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 " :class="isDesktop ? 'des_container mt-3 dt_S_user' : 'no_padding margin_below_app_bar_2'"  :id="$device.isMobile ? 'height_div' : ''">
                <template v-if="l_u_user_show && l_u_user_show.length > 0">
                    <NuxtLink v-for="user in l_u_user_show" :key="user" :to="localePath({ name: 'username', params: { username: user.username } })"
                            @click="save_tracking_user({ placement: 'search' }, { clear: 'clr_old_data' })" class="my_card l_list_company_search">
                        <div class="ml-2 position_relative">
                            <img :src="user.photo && user.photo.small ? user.photo.small.url : empty_img_user"
                                :alt="user.name ? user.name : ''" class="img_profile_list bg_reload_img" @error="imageUrlAlt_profile">
                            <div v-if="user.online_status.is_active" class="l_online_status"></div>
                        </div>
                        <div class="l_search_user_div_style">
                            <div class="con-title-com">
                                <p class="truncate_wrap_2">{{ user.name ? user.name : '' }}</p>
                ...
                <template v-if="!isDesktop">
                    <ClientOnly>
                        <VueEternalLoading :load="search_show_user" :key="reset_scroll" v-model:is-initial="isInitial" margin="800px">
                            <template #loading>
                                <div v-if="l_u_user_show.length > 0" class="text-center mt-4">
                            ...
                            <template #error="{ retry }">
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no_padding" :class="l_u_user_show.length > 0 ? '' : 'margin_below_app_bar'">
                ...
                <template v-else>
                    <div v-if="user_dt_loading" class="width_100 text-center mt-4" ref="targets">
                    <div v-if="l_u_user_show.length > 0" class="text-center mt-4 mb-4">
                        <img :src="asset_url+version_library+'img/newLoading.gif'" alt="Loading..." class="loading_width">
                    </div>
                    <div v-else class="full_width load_f_mg">
                        <div v-for="index in 6" :key="index" class="my_card l_list_company_search">
                    ...
                    <button v-if="user_hide_btn_load_more" @click="manually_search_show_user" class="btn">{{ $t('text_desktop.load_more') }}</button>
                    <div v-else-if="l_u_user_show.length === 0" class="no_more_result n-m-r-mg text-center width_100">
                        <i class="fa fa-exclamation-circle u_no_result_font"></i>
                        <p class="no_more_result mt-2">{{ $t('message.no_result') }}</p>
                    </div>
                    </div>
...
<script setup>
    ...
    const l_u_pageSize = useState('l_u_pageSize', () => 0);
    const l_u_user_show = useState('l_u_user_show', () => []);
    ...
    const request_first = useState('request_first', () => '');  // for request the first, prevent request duplicated.

    // --- manually load more user ---
    const user_hide_btn_load_more = useState('user_hide_btn_load_more', () => true);
    const user_dt_loading = ref(false);

    // --- meta variable ---
    const meta_title_l_user = useState('meta_title_l_user', () => 'Listing Users - Khmer24.com');
    const isMobile= ref(helper.m_or_d())

    if (process.server) {
        await search_show_user_server();
    }

    async function search_show_user_server() {
        try {
            const { data: res_s_u_s } = await useFetch(config.VUE_APP_API_URL + 'profiles/public_users', {
                params: {
                    offset: l_u_pageSize.value,
                    lang: 'en',
            ...
            if (res_s_u_s.value) {
                if (res_s_u_s.value.data.length) {
                    l_u_user_show.value = l_u_user_show.value.concat(res_s_u_s.value.data); // concat in multiple array
                }
                l_u_pageSize.value += res_s_u_s.value.limit; // add offset
                // close btn loadMore in desktop view
                if (isDesktop) { user_hide_btn_load_more.value = l_u_user_show.value.length < l_u_pageSize.value ? false : true; }
            }
        } catch (e) {
    ...
    useHead({
        title: meta_title_l_user.value,
        meta: [
            // -- ios --
            // { name: 'apple-mobile-web-app-title', content: meta_title_l_user.value },
            // -- seo page --
            { name: 'keywords', content: 'Buy, Sell, Khmer24 user, User, Profiles, Profile khmer24, List profile, profile in khmer, khmer24 profile, khmer user, khmer profile' },
            { name: 'description', content: 'Buy, Sell, Find profile in khmer24, Sell your 2nd hand items on Khmer24.com, the Cambodia #1 buy and sell website.' },

            {property: 'og:title', content: meta_title_l_user.value},
    ...
    onMounted(() => {
        if (process.client) {
            ...
            if (isDesktop && l_u_user_show.value.length === 0) {
                manually_search_show_user(); // In Desktop View and post empty for the first, must get new data.
            }
        }
    })

    // ---- search show user ----
    async function search_show_user({loaded, error, noMore, noResults}, { isFirstLoad }) {
        ...
        if (l_u_pageSize.value !== 0 && l_u_user_show.value.length < l_u_pageSize.value) {
            if (l_u_user_show.value.length === 0) {
            ...
            $fetch(config.VUE_APP_API_URL + 'profiles/public_users', {
                params: {
                    offset: l_u_pageSize.value,
                    lang: 'en',
                    keyword: '' + route.query.keyword + '',
                ...
                if (res.data.length) {
                    l_u_user_show.value = l_u_user_show.value.concat(res.data); // concat in multiple array
                }
                l_u_pageSize.value += res.limit; // add offset

                // -- tracking on google analytic --
                if (l_u_user_show.value.length <= 30 && l_u_pageSize.value === 60) {
                } // prevent saved post smaller than 30 and request two time
                else {
                    track_google_analytics(l_u_pageSize.value > 30 ? l_u_pageSize.value - 30 : 0);
                }
                ...
                if (res.data.length === 0) {
                    ...
                    if (l_u_user_show.value.length < l_u_pageSize.value) {
                        noMore(); // close scroll.
                    } else {
                        loaded(); // scroll to down and load more.
                    }
                }
    ...
    // ---- manually get show user ----
    function manually_search_show_user() {
      user_dt_loading.value = true;
      ...
      $fetch(config.VUE_APP_API_URL + 'profiles/public_users', {
        ...
        l_u_pageSize.value += res.limit; // add offset
        if (res.data.length) { l_u_user_show.value = l_u_user_show.value.concat(res.data); } // concat in multiple array
        user_dt_loading.value = false;
        user_hide_btn_load_more.value = l_u_user_show.value.length < l_u_pageSize.value ? false : true;
        ...
        if (!e.response) {
          ...
          if (reload_q_again_1.value <= 2) {
            reload_q_again_1.value += 1;
            setTimeout(() => { manually_search_show_user(); }, 3000);
          } else {
            localStorage.removeItem('auth_user');   // remove localStorage
            VueCookieNext.removeCookie('auth_user');
            router.replace(localePath({name: 'index'}));
    ...
    function track_google_analytics (page) {
        // setTimeout(() => {
            ...
            let data = {
                page_title: meta_title_l_user.value,
                page_location: page_location,
                page_path: page_path,
            }
            // -- event on page --
            event('search_result/users', data);
            // -- page_view --
            pageview(data);
        // },50);
    }
    ...
    // ---- ripple effect on UI ----
    function ripple() {
        setTimeout(function () { $(".arrow_back_nav").ripple(); }, 300);
    }
    // ---- user tracking ----
