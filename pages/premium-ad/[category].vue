...
<template>
    <div :class="!mobileDevice ? 'screen-content-desktop' : ''">
             ...
             <div class="_div_nav bg_app_bar_header_">
                 ...
                 <div class="col pl-0 l_h_con_title">
                     <p class="name_style truncate_wrap mb-0 ml-2 text-left text-capitalize">
                         {{ errorData.message ? $t('account_t.membership') : pageTitle }} 
                     </p>
                 </div>
             </div>
         </header>
         <main >
             <div class="max_width_form"> 
                 <div> 
                     <div class="position-relative" :class="!mobileDevice ? 'px-3' : ''">
                         <!-- Loading state -->
                         <div v-if="isLoading"  :class="mobileDevice ? 'px-3' : ''">
                         ...
                         <div v-else>
                             <div v-if="errorData.status !== null" :class="mobileDevice ? 'pt-5' : 'mt_80'">
                                 <div v-if="errorData.status === 503" class="d-flex flex-column justify-content-center align-items-center " :class="mobileDevice ? 'px-3 pb-3 pt-2 unavailable_service_container' : 'unavailable_service_container_desktop'">
                                     ...
                                     <button type="button" @click="getAdsByCategory"  class="mt-2 mx-auto btn btn-xm btn-k24-primary" :class="mobileDevice ? 'w-100' : 'w-25'"> {{  $t('account_t.retry') }}</button>
                                 </div> 
                                <!-- Error conntection or API not response -->
                                     ...
                                     <div class="col no_padding">
                                         <div class="no_more_result p-2">
                                             <div class="p_bg_status">
                                                 ...
                                                 <button @click="getAdsByCategory" class="btn bg_btn_status"> {{ $t('error_connect.retry') }} </button>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                                 <div v-else class="membership_banner_store_not_found" :class="isDesktop ? 'rounded-2 px-3': 'membership_banner_store_not_found_mobile px-3'" >
                                     ...
                                     <button type="button" :class="!mobileDevice ? 'w-25' : 'w-100'" class="membership_banner_store_not_found_retry_link rounded-3" @click="getAdsByCategory">{{  $t('account_t.retry') }}</button>                    
                                 </div>
                             </div>
                             <div v-else :class="mobileDevice ? 'py-5' : 'main_min_height mt_80'" >
                                 <div :class="mobileDevice ? 'pt-3 px-3 pb-5' : ''"> 
                                     <div  :class="!mobileDevice ? 'pb-5 mb-3' : ''">
                                         <div>
                                             <div v-if="currentActivePlan" class="mb-4">
                                                 <h3 class="mb-1 current_plan_title">{{ $t('account_t.current_plan') }}</h3>
                                                 <h1 class="current_my_sub_plan_title">{{ currentActivePlan && mySubByC && mySubByC.plan_title ?  mySubByC.plan_title : '' }}</h1>
                                             </div>
                                             <div v-else class="mb-4">
                                                 <h1 class="current_my_sub_plan_title" :class="!isDesktop ? 'w-75' : ''" >{{ $t('account_t.category_title') }}</h1>
                                             </div>
                                             <div @click="activePlan = index, focusAndMoveToMiddle(plan[0].plan)" :id="'id'+index" :key="plan" v-for="(plan, index) in planByCategory" class="other_plan_by_category_card" :class="{ 'currentActivePlan': currentActivePlan == index, 'active': activePlan == index}">
                                                 <div @click="choosePlan(plan, index)" :class="isDesktop ? 'other_plan_by_category_card_content' : 'other_plan_by_category_card_content_mobile'">
                                                     ...
                                                     <p class="other_plan_by_category_card_content_description">{{ plan[0].description ? plan[0].description : ''}}</p>
                                                     <div class="_mb_16">
                                                         <div :key="s" v-for="s in plan[0].setting" class="other_plan_by_category_card_content_settings">
                                                             ...
                                                             <span class="other_plan_by_category_card_content_setting_title">{{ s.value ? s.value : 'None' }}</span>
                                                         </div>
                                                     </div>
                                                     <div class="other_plan_by_category_card_term term_active">
                                                         ...
                                                         <div v-if="activePlan == index || currentActivePlan == index" class="other_plan_term_list">
                                                             <div :key="termIndex" v-for="(term, termIndex) in plan" class="other_plan_term_list_item">
                                                                 <label  @click="chooseTerm(index, term, termIndex)" class="other_plan_term_list_item_label" :for="term.id" :class="[currentActiveTermId == term.id ? 'currentActiveTermId' : '']" >
                                                         ...
                                                         <div v-else class="other_plan_term_card_view">
                                                             <label @click="chooseTerm(index, term, termIndex), storeData()" :key="term" v-for="(term, termIndex) in plan" class="other_plan_term_card_view_card" :for="term.id">
                                                                 <p :class="isDesktop ? 'fs-4' : 'fs-6'" class="mb-0 fw-semibold other_plan_term_card_view_card_term">
                                                                     {{ term.term }}
                                                                 </p>
                                                                 <p class="mb-0 other_plan_term_card_view_card_term_type">
                                                                     {{ $t(`account_t.${term.term_type}`)}}{{ term.term_type && ( term.term_type == 'day'|| term.term_type == 'month' )  && term.term > 1 &&  locale == 'en' ? 's' : '' }}
                                                                 </p>
                                         ...
                                         <div v-if="!isLoading && isDesktop" v-show="errorData.message == null && subScribeFormData.planTitle" >
                                             <div class="subscribe_button_footer_wrapper_not_fix"  :class="!mobileDevice ? 'rounded-1' : ''">
                                                 ...
                                                 <button type="button" class="btn form-control-lg btn-xm text-white btn-k24-secondary subscribe_button_footer" @click="storeData(route.params.category), toCheckout()" >
                                                     {{ $t(`account_t.subscribe_now`)}}   
                                                 </button>
                                             </div>
                                         </div>
                                         <!-- Subscribe Button -->
                                         <div v-if="!isLoading && !isDesktop" v-show="errorData.message == null && subScribeFormData.planTitle" class="subscribe_button_footer_wrapper">
                                             <div class="subscribe_button_footer_term_wrapper">
                                                 <div class="subscribe_button_footer_term_left">
                                                     <p class="mb-0 subscribe_button_footer_term_plan">{{ subScribeFormData.planTitle ? $t(`account_t.${subScribeFormData.planTitle}`)+ ':' : '' }} </p>
                                                     <p class="mb-0 subscribe_button_footer_term">{{ subScribeFormData.term ? subScribeFormData.term.term : '' }}</p>
                                             ...
                                             <button type="button" class="btn form-control-lg btn-xm btn-k24-secondary subscribe_button_footer" @click="storeData(route.params.category), toCheckout()" >
                                                 {{ $t(`account_t.subscribe_now`)}}       
                                             </button>
                                         </div>
                                     </div>
 ...
 const filterCurrentPlan = () => {        
    if(mySubByC.value) {
         currentActivePlan.value = mySubByC.value.plan
         currentActiveTermId.value = mySubByC.value.plan_id 
         focusAndMoveToMiddle(mySubByC.value.plan)
    } else if (scollToPlan.value) {
        
        focusAndMoveToMiddle(scollToPlan.value)
    } 

    if(currentActivePlan.value) {
        activePlan.value = currentActivePlan.value
        subScribeFormData.planTitle = mySubByC.value.plan
        subScribeFormData.term = mySubByC.value
        subScribeFormData.termId = mySubByC.value.plan_id
     } 
 }
 ...
 async function getAdsByCategory() {
     ...
     try {
         ...
         const response = await $fetch(`${config.VUE_APP_API_URL_PAYMENT}services/ads/${route.params.category}?lang=${locale.value}`, {
             retry: 2, 
             retryDelay: 3000, 
             method: "GET", 
             query: query , 
         ...
         if(response) {
            ...
            if (response.subscription_plan) {
                mySubByC.value = response.subscription_plan
            }
            ...
            for (const p in response.data) {
                if (response.data[p] && response.data[p].recommended && response.data[p].recommended == true) {
                    ...
                    // for scroll to
                    scollToPlan.value = response.data[p].plan
                }
            }
 
            planByCategory.value = groupedPlans
         ...
         if (!error.response) {
             ...
             if(error.response.status === 401) {
                 if(attemptRequest.value < 2) {
                     ...
                     setTimeout(() => { getAdsByCategory() }, 3000);
                 } else {
...
function choosePlan(plan, index){
 ...
 // Choose term if click on each term 
 const chooseTerm = (index, t) => {
     subScribeFormData.planTitle = index
 ...
 function toCheckout(){
     ...
     return router.push(localePath({ name: 'checkout', query: queries }))
 }
 
 // Store subscribe data to localStorage
 const storeData = (plan_category=null) => { 
     const cardData = localStorage.getItem('cart_data') ? JSON.parse(localStorage.getItem('cart_data')) : null
     localStorage.setItem('cart_data', JSON.stringify({'id': cardData.id, 'type': 'ads', 'plan': subScribeFormData.termId, 'category': plan_category }))
 }
