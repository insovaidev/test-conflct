                ...
                <div v-else>
                    <div v-if="errorData.status !== null" :class="mobileDevice ? 'mt-5' : 'mt_80'">
                        <div v-if="errorData.status === 503" class="d-flex flex-column justify-content-center align-items-center " :class="mobileDevice ? 'px-3 pb-3 pt-2 unavailable_service_container' : 'unavailable_service_container_desktop'">
                            ...
                            <button type="button" @click="getPremiumAccountPlan"  class="mt-2 mx-auto btn btn-xm btn-k24-primary" :class="mobileDevice ? 'w-100' : 'w-25'"> {{  $t('account_t.retry') }}</button>
                        </div>
                            ...
                            <div class="col no_padding">
                                <div class="no_more_result p-2">
                                    <div class="p_bg_status">
                                        ...
                                        <button @click="getPremiumAccountPlan" class="btn bg_btn_status"> {{ $t('error_connect.retry') }} </button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-else class="membership_banner_store_not_found" :class="!mobileDevice ? 'rounded-2 px-3': 'px-3 membership_banner_store_not_found_mobile'">
                            ...
                            <button type="button" :class="!mobileDevice ? 'w-25' : 'w-100'" class="membership_banner_store_not_found_retry_link rounded-3" @click="getPremiumAccountPlan">{{$t(`account_t.retry`)}}</button>
                        </div>
                    </div>

                    <div v-else :class="mobileDevice ? 'py-5' : 'main_min_height mt_80 px-3'">
                        <div :class="mobileDevice ? 'px-3 pt-2' : ''">
                            <h1 v-if="plans && plans.data" class="mb-0 fs-5 premium_title_for_increase" :class="mobileDevice ? 'fw-medium' : 'w-100'">{{$t('account_t.premium_plan_title') }}</h1>
                            <h2 v-if="plans && plans.data" class="premium_title_choose_category mb-3" :class="!mobileDevice ? 'mb-2' : '_mb_16 fw-light'">{{$t('account_t.premium_plan_des') }}</h2>
                            <!-- Current Plan -->
                            <div v-if="plans.subscription_plans" class="current_premuim_plan_card_wrapper">
                                ...
                                <NuxtLink :key="sub" v-for="sub in plans.subscription_plans" :to="localePath({ name: 'premium-account-category', params: { category: sub.category_type }, query: { id: 'me' }})" class="current_premuim_plan_card" :class="!mobileDevice ? 'card_padding_desktop' : 'card_padding_mobile'" >
                            ...
                            <div v-if="plans && plans.data" class="orther_premuim_plan_card_wrapper">
                                ...
                                <div class="premium_plan_cards" :key="plan" v-for="plan in plans.data" >
                                    <NuxtLink
                                        @click="toPremiumPlanByCategory(plan)"
                                        class="other_premuim_plan_card" :class="[plan.category_type == route.query.category_change ? 'selected' : '',  !mobileDevice ? 'card_padding_desktop' : 'card_padding_mobile']"  >
...
useHead({
    title: 'Premium Account - Khmer24.com',
    meta: [
...
const toPremiumPlanByCategory = (plan) => {
    if(plan) {
        router.push(localePath({ name: 'premium-account-category', params: { category: plan.category_type }, query: { 'id': 'me' }}))
    }
}
...
async function getPremiumAccountPlan() {
    ...
    try {
        ...
        const response = await $fetch(`${config.VUE_APP_API_URL_PAYMENT}services/account`, {
            retry: 2, 
            retryDelay: 3000, 
            query: query,
            method: "GET", 
        ...
        if(!error.response) {
            ...
            if(error.response.status === 401) {
                if(attemptRequest.value < 2) {
                    ...
                    setTimeout(() => { getPremiumAccountPlan() }, 3000);
                } else { 
