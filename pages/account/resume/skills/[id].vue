    ...
    <div>
        ...
        <div class="form-style" :class="isMobile ? ' pt-5 bg_style' : 'min_h'">
            ...
            <div :class="isMobile ? 'p-3' : 'form_input p-3 bg-white mt_80'">
                <!-- <h4 class="screen-d">{{ $t('resume_cv.skill') }}</h4> -->
                <div v-if="!isMobile" class="dt_title_header px-0 pt-0 mb-3"><h1>{{ $t('resume_cv.skill') }}</h1></div>
                <form role="form">
                    <div>
                        <div class="mb-3">
                            <label for="title" class="label_style label mb-1 full_width text-left">{{ $t('resume_cv.title') }}<span class="text-danger">*</span></label>
                            <input type="text" v-model="GetSkill.title" class="form-control more_border" :class="isMobile ? 'field-height' : 'dt-field-height'" id="title" name="title">
                        </div>
                        <div class="mb-3 botton-style">
                            <label for="level" class="label_style label mb-1 full_width text-left">{{ $t('resume_cv.level') }}<span class="text-danger">*</span></label>
                            <select v-if="GetSkill.level" v-model="GetSkill.level.value" class="form-control more_border" :class="isMobile ? 'field-height' : 'dt-field-height'" id="level" name="level">
                                <option hidden></option>
                                <option value="basic">{{ $t('resume_cv.basic') }}</option>
                                <option value="intermediate">{{ $t('resume_cv.intermediate') }}</option>
                                <option value="advanced">{{ $t('resume_cv.advanced') }}</option>
                                <option value="expert">{{ $t('resume_cv.expert') }}</option>
                            </select>
...
const GetSkill = useState('GetSkill',()=>'')
...
function get(){
    loadindScreen.value = true
    $fetch(config.VUE_APP_API_URL_JOB+'me/resume/skills/'+route.params.id+'', { 
        headers: { "Access-Token": auth_users.value.tokens.access_token }
    }).then(res=>{
        GetSkill.value = res.data
    }).catch(error=>{
    ...
    }).finally(()=>{ setTimeout(()=>{ loadindScreen.value = false },250) })
}
// ============= function input =============
function saveData(){
    check_new_auth_user()
    ...
    const data_input = {
        title: GetSkill.value.title,
        level: GetSkill.value.level.value
    }
    $fetch(config.VUE_APP_API_URL_JOB+'me/resume/skills/'+route.params.id+'', {
