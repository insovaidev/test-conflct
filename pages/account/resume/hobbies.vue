    ...
    <div>
        <div class="form-style" :class="isMobile ? 'pt-5 bg_style' : 'min_h'">
            ...
            <div v-else :class="isMobile ? 'p-3' : 'form_input p-3 bg-white mt_80'" >
                <div v-if="!isMobile" class="dt_title_header px-0 pt-0 mb-3"><h1>{{ $t('resume_cv.habbies') }}</h1></div>
                <div class="mb-3">
                    <label for="" class="label_style label full_width text-left">{{ $t('resume_cv.habbies') }}</label>
                    <textarea v-model="Hobbies" class="form-control more_border" rows="10"></textarea>
                </div>
...
function get(){
    if(auth_users.value){
        Loading.value = true
        $fetch(config.VUE_APP_API_URL_JOB+'me/resume/hobbies', {
...
function saveData(){
    ...
    const data_input = {
        hobbies: Hobbies.value != undefined ? Hobbies.value: '',
    }
    $fetch(config.VUE_APP_API_URL_JOB+'me/resume/hobbies', {
