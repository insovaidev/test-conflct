<template>
    <div :class="!mobileDevice ? 'screen-content-desktop' : ''">
        ...
        <main class="max_width_form main_min_height" :class="!mobileDevice ? 'mt_80': ''">
            ...
            <div>  
                ...
                <div v-else>
                    <form class="bg-white" :class="!mobileDevice ? 'border border-1 rounded pt-sm-2' : 'mt-5 pt-2'">
                        <div class="px-4 mb-3">
                            <div v-show="!mobileDevice && mobileDevice != null " class="mb-3 pt-3">
                                <h1 class="fw-semibold fs-5">{{ $t('account_t.add_address') }}</h1>
                                <hr/>
                            </div>
                            <div class="mb-3">
                                <label for="name" class="label_style mb-1 full_width text-left">{{ $t('resume_cv.name') }}<span class="text-danger">*</span></label>
                                <input type="text" v-model="formData.name" id="name" name="name" class="form-control height more_border " :class="mobileDevice ? 'field-height' : 'dt-field-height'" >
                            </div>
                            <div>
                                <label for="company" class="label_style mb-1 full_width text-left">{{ $t('resume_cv.company') }}</label>
                                <input type="text" id="company" name="company" v-model="formData.company" class="form-control more_border " :class="mobileDevice ? 'field-height' : 'dt-field-height'">
                            </div>
                        </div>
                        <div class="px-4 mb-3">
                            <div class="mb-3">
                                <label for="email" class="label_style mb-1 full_width text-left">{{ $t('resume_cv.email') }}</label>
                                <input type="email" v-model="formData.email" id="email" name="email" class="form-control more_border " :class="mobileDevice ? 'field-height' : 'dt-field-height'">
                            </div>
                            <div>
                                <label class="label_style mb-1 full_width text-left">{{ $t('resume_cv.phone') }}<span class="text-danger">*</span></label>
                        ...
                        <div class="px-4 mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" id="address" v-model="formData.address" class="form-control more_border " name="address" :class="mobileDevice ? 'field-height' : 'dt-field-height'">
                            </div>
                        ...
                        <div class="px-4 mb-3">
                            <div >
                                ...
                                <input v-model="formData.tax_id" type="text" id="tax_id" class="form-control " :class="mobileDevice ? 'field-height' : 'dt-field-height'">
                                ...
                                <div class="d-flex align-items-center justify-content-between border border-1 rounded-3 px-3 py-1" @click="toggleCheckbox" :class="mobileDevice ? 'field-height' : 'dt-field-height'">
                                    ...
                                    <div class="">
                                        <label for="change_auto_up_profile" class="s_t_switch" >
                                            <input type="checkbox" v-model="is_default" name="change_auto_up_profile"  @click="toggleCheckbox" id="change_auto_up_profile" >
                        ...
                        <div class="px-4 py-3">
                            <div v-if="isDesktop">
                                <button class="btn form-control-xm btn-xm btn-k24-secondary w-100" @click="er_"> {{ locale==='km' ? 'បញ្ជូន' : 'Submit' }} </button>
                            </div>
                            <div v-if="!isDesktop">
                                <button class="btn form-control-lg btn-xm btn-k24-secondary w-100" @click="er_"> {{ locale==='km' ? 'បញ្ជូន' : 'Submit' }} </button>
                            </div>                    
                        </div>
                    </form>
                </div>
                        ...
                        <div>
                            <ul class="list-unstyled bg-white">
                                <li v-for="loc in mul_arr_location" :key="loc" class="full_field_tag cur_sur h-v-mouse list_location_ov_border" @click="show_next_location(loc)">
                                    ...
                                    <i class="float-end" :class="loc.id === old_arr_loc.loc || loc.id === old_arr_loc.dis || loc.id === old_arr_loc.com ? 'fas fa-check-circle color_sub_icon mt-1 parent_color' : 'ion-ios-arrow-forward text_grey'"></i>
                                </li>
                                <br><br><br>
                            </ul>
                        </div>
    ...
    <div class="modal fade" id="show_modal_google_map" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-fullscreen" :class="isDesktop ? 'vh-md-100 d-md-flex edit_billing_address_modal_dialog_desktop' : 'modal-fullscreen edit_billing_address_modal_dialog_monile'">
...
useHead({
    title: 'Add Billing Address - Khmer24.com', 
    meta: [
        // -- seo page --
        { name: 'keywords', content: 'Buy, Sell' },
        { name: 'description', content: 'Buy, Sell' },

        { property: 'og:title', content: 'Add Billing Address - Khmer24.com' },
