<template>
        ...
        <div v-if="isMobile" class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no_padding fix_app_bar">
            <div class="_div_nav bg_app_bar_header_">
                ...
                <div class="col pl-0 l_h_con_title">
                    <p v-if="data_edit" class="name_style truncate_wrap mb-0 ml-2 text-left">
                        {{ locale=== 'km' ? 'កែប្រែអាស័យដ្ឋាន' : 'Edit Address' }}
                    </p>
                    <p v-else class="name_style truncate_wrap mb-0 ml-2 text-left">
                        {{ locale=== 'km' ? 'បន្ថែមអាស័យដ្ឋាន' : 'Add Address' }}
                    </p>
                </div>
            </div>
        </div>
        <div class="content-resume mb-5 mt_80">
            <form>
                <div class="bg-white p-4 mb-2" :class="isMobile ? 'mt-5' : ''">
                    <template v-if="!isMobile">
                        <div v-if="data_edit" class="dt_title_header px-0 pt-0 mb-3"><h1> {{ locale=== 'km' ? 'កែប្រែអាស័យដ្ឋាន' : 'Edit Address' }}</h1></div>
                        <div v-else class="dt_title_header px-0 pt-0 mb-3"><h1> {{ locale=== 'km' ? 'បន្ថែមអាស័យដ្ឋាន' : 'Add Address' }}</h1></div>
                    </template>
                    <div class="mb-3">
                        <strong>{{ locale==='km' ? 'ពត៌មានផ្ទាល់ខ្លួន' : 'Personal Information' }}</strong>
                    </div>
                    <div class="mb-3">
                        <label for="" class="label_style mb-1 full_width text-left">{{ $t('resume_cv.name') }}<span class="text-danger">*</span></label>
                        <input type="text" v-model="formData.name" name="name" class="form-control more_border " :class="isMobile ? 'field-height' : 'dt-field-height'">
                    </div>
                    <div>
                        <label for="" class="label_style mb-1 full_width text-left">{{ $t('resume_cv.company') }}</label>
                        <input type="text" v-model="formData.company" class="form-control more_border " :class="isMobile ? 'field-height' : 'dt-field-height'">
                    </div>
                </div>
                <div class="bg-white p-4 mb-2">
                    <div class="mb-3">
                        <strong>{{ locale==='km' ? 'ទំនាក់ទំនង' : 'Contact' }}</strong>
                    </div>
                    <div class="mb-3">
                        <label for="" class="label_style mb-1 full_width text-left">{{ $t('resume_cv.email') }}</label>
                        <input type="email" v-model="formData.email" id="email" class="form-control more_border " :class="isMobile ? 'field-height' : 'dt-field-height'">
                    </div>
                    <div>
                        <label for="" class="label_style mb-1 full_width text-left">{{ $t('resume_cv.phone') }}<span class="text-danger">*</span></label>
                        <template v-for="(p_n, index) in phone" :key="p_n">
                            ...
                            <div :class="phone.length === 2" class="w-100 position-relative">
                                <input v-model="formData.phone[1]" type="text" inputmode="numeric" :placeholder="$t('message.phone_2')" maxlength="10" id="phone2" name="phone_1" ref="inputapply1" class="form-control more_border phone[1]" :class="isMobile ? 'field-height' : 'dt-field-height'" />
                                <i @click="minus_field_phone(index)" class="fas fa-minus-circle text-danger icon_in_text_field cur_sur" :class="isMobile ? 'pad_icon_minus' : 'p-2 fn_22'"></i>
                            </div>
                ...
                <div class="bg-white p-4 mb-2">
                    <div class="mb-3">
                        <strong>{{ locale==='km' ? 'អាស័យដ្ឋាន' : 'Address' }}</strong>
                    </div>
                    <div class="mb-3 position_relative">
                        <label for="location" class="label_style mb-1 label text-left full_width"> {{ $t('new_text.location') }}<span class="text-danger">*</span> </label>
                        <div @click="choose_location('show')" id="location" name="location" :class="province_name ? (isMobile ? 'p_sel_1 field-height' : 'p_sel_1 dt-field-height') : (isMobile ? 'p_sel_2 field-height' : 'p_sel_2 dt-field-height')" class="mb-0 e_selection text-left cur_sur  pt-0">
                            <p v-if="province_name" class="truncate font_15 pad_dis_1 mt-2 width_90">
                                {{ province_name ? province_name : '' }}
                                {{ district_name ? ' , '+district_name : '' }}
                                {{ commune_name ? ' , '+commune_name : '' }}
                            </p>
                            <p v-if="!province_name" class="m-2 text_grey">{{ $t('new_text.choose_location') }}</p>
                        </div>
                        <i class="ion-android-arrow-dropdown s_icon_in_select_option" :class="isMobile ? '' : 'pt-1'" id="t_i" style="margin-top: -58px!important;"></i>
                    ...
                    <div class="mb-3">
                        <label for="" class="label_style mb-1 full_width text-left">{{ $t('resume_cv.address') }}<span class="text-danger">*</span></label>
                        <input type="text" v-model="formData.address" class="form-control more_border " :class="isMobile ? 'field-height' : 'dt-field-height'" name="address">
                    </div>
                ...
                <div class="bg-white p-4 mb-2">
                    <div class="mb-3">
                        <strong>{{ locale==='km' ? 'ព័ត៌មានអំពីពន្ធ' : 'Tax detail' }}</strong>
                    </div>
                    <div >
                        <label for="" class="label_style mb-1 full_width text-left">Tax ID</label>
                        <input v-model="formData.tax_id" type="text" id="tax_id" class="form-control more_border " :class="isMobile ? 'field-height' : 'dt-field-height'">
                        <span v-if="tax_er" class="err_field_address">{{ tax_er }}</span>
                    </div>
                </div>
                <div class="bg-white p-4 mb-2">
                    <div class="mb-3">
                        <strong>{{ locale==='km' ? 'រក្សាទុកជា' : 'Save as' }}</strong>
                    </div>
                    <div class="category">
                        <div class="radio-new-or-second-hand text-center row ">
                            <div class="col p_pad_radio_option" @click="other('home')">
                                ...
                                <label for="home"  class="form-control more_border" :class="isMobile ? 'field-height' : 'dt-field-height'" id="home_l"><p class="m-0 " :class="isMobile ? '' : 'mt-0'">{{ locale==='km' ? 'ផ្ទះ' : 'Home' }}</p></label>
                            </div>
                            <div class="col p_pad_radio_option" @click="other('work')">
                                ...
                                <label for="work" class="form-control more_border label_work" :class="isMobile ? 'field-height' : 'dt-field-height'" id="work_l"><p class="m-0 " :class="isMobile ? 'pt-1' : 'mt-0'">{{ locale==='km' ? 'កន្លែងធ្វើការ' : 'Work' }}</p></label>
                            </div>
                            <div class="col p_pad_radio_option" @click="other('other')">
                                ...
                                <label for="other" class="form-control more_border" :class="isMobile ? 'field-height' : 'dt-field-height'" id="other_l"><p class="m-0 " :class="isMobile ? 'pt-1' : 'mt-0'">{{ locale==='km' ? 'ផ្សេងទៀត' : 'Other' }}</p></label>
                            </div>
                        </div>
                    </div>
                    <div id="other_id" hidden class="mt-4">
                        <input type="text" v-model="formData.label"  class="form-control more_border " :class="isMobile ? 'field-height' : 'dt-field-height'" name="label" :placeholder=" locale==='km' ? 'រក្សាទុកជា' : 'Save as' ">
                    </div>
                    <div class=" mt-4">
                        <div class="d-flex g_set_default border" :class="isMobile ? 'field-height' : 'dt-field-height'" @click="toggleCheckbox">
                            <div :class="isMobile ? '' : 'pt-1'" class="set_default">{{ locale==='km' ? 'កំណត់ជាជំរើសទីមួយ' : 'Set as default' }}</div>
                            <div class="col g_switch" :class="isMobile ? 'field-height' : 'pt-1 dt-field-height'">
                ...
                <template v-if="isMobile">
                    <div class="nav p_pad_btn_ap_job" @click="er_" >
                        <button class="btn btn-k24-secondary" :class="isMobile ? 'btn-height' : 'dt-btn-height'"> {{ locale==='km' ? 'បញ្ជូន' : 'Submit' }} </button>
                    </div>
                </template>
                <template v-else>
                    <div class="mt-3" @click="er_" >
                        <button class="btn btn-k24-secondary" :class="isMobile ? 'btn-height' : 'dt-btn-height'"> {{ locale==='km' ? 'បញ្ជូន' : 'Submit' }} </button>
                    </div>
                </template>
            </form>
    ...
    <div class="modal fade" :class="isMobile ? 'style_full_screen' : ''" id="show_modal_location" tabindex="-1" role="dialog">
        <div class="modal-dialog " :class="isMobile ? 'modal_dialog_style' : 'modal-dialog-scrollable m-auto m_w' " role="document">
            <div class="modal-content " :class="isMobile ? 'modal_content_style' : 'rounded'">
                ...
                <div class="modal-body" :class="isMobile ? 'de-head-quick_chat m_bottom_app_bar' : 'p-0 '">
                    <div>
    ...
    <div class="modal fade" :class="isMobile ? 'style_full_screen' : ''" id="show_modal_google_map" tabindex="-1" role="dialog">
        <div class="modal-dialog " :class="isMobile ? 'modal_dialog_style' : 'modal_dialog_styles' " role="document">
            <div class="modal-content " :class="isMobile ? 'animate-bottom' : 'rounded'">
                <div class="" :class="isMobile ? 's_content overflow_class' : ''">
                    <!-- content -->
    ...
    <div class="toast style_toast d-none">
        <div class="toast-header header_padding_toast">
            <strong class="mr-auto style_flex">
                ...
                <div v-if="message_er" class="m-0 ml-2 font_15 text-danger">
                    {{ message_er }}
                </div>
            </strong>
...
function saveData(){
    ...
    if(window.navigator.onLine){
        ...
        $fetch(url_addr.value+'?storeid=0', {
...
function choose_location(con) {
    if (window.navigator.onLine) {
        ...
        $('#show_modal_location').modal('show');
...
function close(){
    $('#show_modal_location').modal('hide');
}
